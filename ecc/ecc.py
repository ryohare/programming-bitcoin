...
from utils.utils import encode_base58_checksum, hash160
...
# public key class of the signature
# really the coordinate in form of x,y
class S256Point(Point):
    ...
    def verify(self, z, sig):
        # femat's little theroem to get 1/s
        s_inv = pow(sig.s, N - 2, N)

        # u=z/s N is the group order number
        u = z * s_inv % N

        # v=r/s N is the group order number
        v = sig.r * s_inv % N

        # uG + vP should be R
        total = u * G + v * self

        # check that the mapped x coord is infact r
        return total.x.num == sig.r

    # SEC serialization of the public key
    def sec(self, compressed=True) -> bytes:
        '''
            returns the binary version in SEC format
        '''
        if compressed:
            # compression just sends over the wire the sign of the
            # y coord and it is then calculated using the provided x
            if self.y.num % 2 == 0:
                # even number
    ...
    def parse(self, sec_bin):
        ...
        # check the evenness of the first byte 
        is_even = sec_bin[0] == 2

        x = S256Field(
            int.from_bytes(sec_bin[1:], 'big')
        )
        ...
        # by taking the sqrt of y
        beta = alpha.sqrt()

        # determine the evenness and return the correct point
        if beta.num % 2 == 0:
            even_beta = beta
    ...
    # helper fucntion to do the hash160
    def hash160(self, compressed=True):
        return hash160(self.sec(compressed))

    # generate the base58 address
...
class Signature:
    ...
    def der(self):
        ...
        res = ''.join(format(x, '02x') for x in rbin)
        print(res)
        # remove all the null byptes at the begining
        ...
        # if sbin has a high bit, add 0x00
        if sbin[0] % 0x80:
            sbin = b'\x00' + sbin


        pbytes = bytes( [2, len(sbin)] ) + sbin
        print(pbytes.hex())
        print(result.hex())
        result += pbytes
