import hashlib
from ecc.ecc import S256Point, Signature
...
def op_dup(stack):
    # have to have a least on instruction on the stack to allow the dup
    if len(stack) < 1:
        return False

    # Append the last element on the stack to the end
    stack.append(stack[-1])

    # return true that the op was successful
    return True
...
def op_hash160(stack):
    # make sure there is something in the stack to hash
    if len(stack) < 1:
        return False

    # get the value to hash off the top of the stack
    element = stack.pop()

    # hash the value and put it on the top of the stack
    stack.append(hash160(element))

    # return true, the op was successful
    return True
...
def op_checksig(stack, z):
    # make sure we received 2 arguments pushed onto the stack
    if len(stack) < 2:
        return False
    
    # get the public key and the signatre
    sec_pubkey = stack.pop()

    # not sure why we need to use the -1 indexer here
    # this is actually the sig hash field which needs to be
    # stripped of the signature prior.
    der_signature = stack.pop()[:-1]

    # validate the signature with the ecc library
    try:
        # exceptions will be thrown on invalid values
        point = S256Point.parse(sec_pubkey)
        sig = Signature.parse(der_signature)
    except(ValueError, SyntaxError) as e:
        return False
    
    # at this point, we have the parsed signature and the public key
    # now we need to ensure that the public key is on the same curve
    # as the signature
    if point.verify(z, sig):
        ...
        # otherwise, the signature is not valid for the provided public key
        stack.append(encode_num(0))
...
def op_checkmultisig(stack, z):
    # check the stack size first to make sure there are instructions
    if len(stack) < 1:
        return False

    # get the n of the m of n multisig instruction
    n = decode_num(stack.pop())

    # make sure the stack has enough entries to support the op we just read off
    if len(stack) < n + 1:
        return False

    # pop off all the n pub keys into an array 
    sec_pubkeys = []
    for _ in range(n):
        sec_pubkeys.append(stack.pop())
    
    # get the number of mutlisigs required
    m = decode_num(stack.pop())

    # check that we have enough m signatures for the op
    if len(stack) < m + 1:
        return False

    # get a list of the der signatures required of n to
    # to do the operation 
    der_sigantures=[]
    for _ in range(m):
        # the [:-1] is to pull off the stack the sighash flag
        # which this function assumes is SIGHASH_ALL
        der_sigantures.append(stack.pop()[:-1])
    
    # pop off the "off by 1" error element in the stack
    stack.pop()

    # signatures need to be in order for the der encoding of this

    # now we need to verify that there are n of m valid signatures
    try:
        # use a generator function to create an array of pub_key_points
        pub_key_points = [S256Point.parse(sec) for sec in sec_pubkeys]
        sigs = [Signature.parse(der) for der in der_sigantures] 

        for sig in sigs:
            # check that we have at least 1 valid public key here
            # add a 0 to the stack using encode_num(0)
            # bail early if we don't have any points left
            if len(pub_key_points) == 0:
                
                # add a 0 to the stack using encode_num(0)
                stack.append(encode_num(0))
                
                # return True because the function was successful
                # but the stack is not going to evaluate to true
                return True

            # iterate over the list of pub_key_points and compare each
            # against the signatures 
            while pub_key_points:
                # grab the first public keys
                pub_key_point = pub_key_points.pop(0)

                # found a match for the supplied pub_key for one of the signatures
                if pub_key_point.verify(z, sig):
                    break
